...
function SystemFlowDiagram({ darkMode }) {
  ...
  return (
    <div className="overflow-x-auto">
      <svg viewBox={`0 0 ${totalW} ${viewH}`} className="w-full" style={{ minWidth: '700px' }}>
        <defs>
          <filter id="clayShadow" x="-30%" y="-30%" width="160%" height="160%">
            <feDropShadow dx="4" dy="5" stdDeviation="0" floodColor="#000000" floodOpacity="0.22" />
          </filter>
        </defs>

        {/* connecting lane */}
        <line x1={nodeW / 2} y1={y + nodeH / 2} x2={totalW - nodeW / 2} y2={y + nodeH / 2} stroke={laneColor} strokeWidth="6" strokeLinecap="round" />
        ...
        {nodes.map((n, i) => {
          ...
          return (
            <g key={n.key}>
              {i > 0 && (
                <polygon
                  points={`${x - gap / 2 - 6},${y + nodeH / 2 - 8} ${x - gap / 2 + 6},${y + nodeH / 2} ${x - gap / 2 - 6},${y + nodeH / 2 + 8}`}
                  fill={laneColor}
                />
              )}
              <rect x={x} y={y} width={nodeW} height={nodeH} rx="26" fill={n.color} filter="url(#clayShadow)" />
              <text x={x + nodeW / 2} y={y + 44} textAnchor="middle" fontSize="14" fontWeight="900" fill="#FFFFFF">
                {n.label.length > 20 ? n.label.split(' ').slice(0, Math.ceil(n.label.split(' ').length/2)).join(' ') : n.label}
              </text>
              {n.label.split(' ').length > 2 && (
                <text x={x + nodeW / 2} y={y + 62} textAnchor="middle" fontSize="14" fontWeight="900" fill="#FFFFFF">
                  {n.label.split(' ').slice(Math.ceil(n.label.split(' ').length/2)).join(' ')}
                </text>
              )}
              <text x={x + nodeW / 2} y={y + nodeH - 16} textAnchor="middle" fontSize="10.5" fontWeight="700" fill="#FFFFFF" opacity="0.85">
                {n.sub}
              </text>
            </g>
          );
        ...
        {/* feedback loop back to start */}
        <path
          d={`M ${totalW - nodeW / 2} ${y + nodeH + 6} C ${totalW - nodeW / 2} ${viewH - 20}, ${nodeW / 2} ${viewH - 20}, ${nodeW / 2} ${y + nodeH + 6}`}
          fill="none" stroke={laneColor} strokeWidth="4" strokeDasharray="2 8" strokeLinecap="round"
        />
        <text x={totalW / 2} y={viewH - 4} textAnchor="middle" fontSize="11" fontWeight="700" fill={subColor}>
          Reflected back into future Needs planning — the cycle continues
        </text>
      </svg>
    </div>
  );
