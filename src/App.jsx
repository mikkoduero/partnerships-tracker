...
// --- RICH SEED DATA WITH SYSTEM TRAILS ---
const initialNeeds = [
...
const initialUsers = [
  { id: 1, name: 'Juan Dela Cruz', username: 'juan.super', email: 'juan.super@deped.gov.ph', role: SYSTEM_ROLES.SUPERADMIN, position: 'Chief Admin Officer', office: 'Regional Office' },
  { id: 2, name: 'Althea Ramos', username: 'althea.ict', email: 'althea.ict@deped.gov.ph', role: SYSTEM_ROLES.ICT_USER, position: 'ITO I', office: 'Regional Office' },
  { id: 3, name: 'Mark Espera', username: 'mark.focal', email: 'mark.focal@deped.gov.ph', role: SYSTEM_ROLES.FOCAL, position: 'PDO II', office: 'Leyte' }
];
...
export default function App() {
  ...
  const handleLogoutSim = () => {
    alert("Simulation System: Clear identity token. Handshaking complete.");
  };
    ...
    <div className={`min-h-screen font-sans flex flex-col md:flex-row transition-colors duration-200 ${
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Header 
          ...
          availableMockUsers={users} 
          darkMode={darkMode} 
          setDarkMode={setDarkMode} 
          onLogout={handleLogoutSim} 
        />
...
function Sidebar({ activeTab, setActiveTab, darkMode, role }) {
  ...
  return (
    <aside className={`w-full md:w-64 border-b md:border-b-0 md:border-r transition-all ${
      darkMode ? 'bg-emerald-950 border-emerald-900/40 text-emerald-100' : 'bg-emerald-900 border-emerald-950 text-white'
    } flex flex-col`}>
...
function Header({ currentUser, setCurrentUser, availableMockUsers, darkMode, setDarkMode, onLogout }) {
  return (
    <header className="px-6 py-4 border-b dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      <div>
        <span className="text-[10px] uppercase font-bold tracking-widest text-emerald-700 dark:text-amber-400">Identity Context Simulation Engine</span>
        <div className="flex items-center gap-2 mt-0.5">
          <select value={currentUser.id} onChange={(e) => setCurrentUser(availableMockUsers.find(u => u.id === Number(e.target.value)))} className="text-xs bg-slate-100 dark:bg-slate-800 font-bold p-1 rounded border dark:border-slate-700 text-emerald-800 dark:text-amber-400 outline-none">
            {availableMockUsers.map(u => <option key={u.id} value={u.id}>{u.name} [{u.role}]</option>)}
          </select>
        </div>
      </div>
...
function SystemFilters({ filters, setFilters, darkMode, includeCategoryFilters = false }) {
  const updateF = (k, v) => {
    setFilters(prev => {
      const state = { ...prev, [k]: v };
      if (k === 'level') { state.office = ''; state.fd = ''; state.section = ''; }
  ...
  return (
    <div className={`p-4 rounded-xl border mb-6 shadow-sm ${darkMode ? 'bg-slate-900/40 border-slate-800' : 'bg-white border-slate-200'}`}>
      <div className={`grid grid-cols-2 sm:grid-cols-4 ${includeCategoryFilters ? 'lg:grid-cols-5' : 'lg:grid-cols-7'} gap-3`}>
        <select value={filters.level} onChange={e=>updateF('level', e.target.value)} className={css}>
          <option value="All">All Operations Hierarchy</option>
          <option value="RO">Regional Level</option>
          <option value="SDO">Division Level</option>
        </select>
        
        <select value={filters.office} onChange={e=>updateF('office', e.target.value)} className={css}>
          <option value="">Office</option>
          {(filters.level === 'RO' ? ['Regional Office'] : filters.level === 'SDO' ? OFFICES.filter(o=>o!=='Regional Office') : OFFICES).map(o => <option key={o} value={o}>{o}</option>)}
        </select>
...
function Dashboard({ needs, contributions, userContext, darkMode }) {
  const [filters, setFilters] = useState({ level: 'All', office: '', fd: '', section: '', year: 'All', quarter: 'All', month: 'All' });

  const { filteredNeeds, filteredConts } = useMemo(() => {
    const filterRecordSet = (list) => list.filter(item => {
      const d = new Date(item.dateLogged);
      if (filters.level !== 'All') {
        const isRO = item.office === 'Regional Office';
        if (filters.level === 'RO' && !isRO) return false;
        if (filters.level === 'SDO' && isRO) return false;
      }
  ...
  const officeStandings = useMemo(() => {
    ...
    filteredNeeds.forEach(n => { if(summary[n.office]) summary[n.office].needs += Number(n.value); });
    filteredConts.forEach(c => { if(summary[c.office]) summary[c.office].conts += Number(c.value); });
  ...
  return (
    <div className="space-y-6">
      <SystemFilters filters={filters} setFilters={setFilters} darkMode={darkMode} />

      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className={containerStyle}>
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Total Needs (Value)</span>
      ...
      <div className={containerStyle}>
        <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4">Ranking of Offices: % Needs vs Contributions</h3>
...
function NeedsWorkspace({ needs, setNeeds, userContext, darkMode }) {
  const [filters, setFilters] = useState({ level: 'All', office: '', fd: '', section: '', category: '', specificItem: '' });
  const [addModal, setAddModal] = useState(false);
  const [editModal, setEditModal] = useState(null);
  const [trailModal, setTrailModal] = useState(null);
  
  const [meta, setMeta] = useState({ office: '', fd: '', section: '' });
  ...
  const currentFilteredView = useMemo(() => {
    return needs.filter(n => {
      if (filters.level !== 'All' && ((filters.level === 'RO' && n.office !== 'Regional Office') || (filters.level === 'SDO' && n.office === 'Regional Office'))) return false;
  ...
  return (
    <div className="space-y-6">
      <SystemFilters filters={filters} setFilters={setFilters} darkMode={darkMode} includeCategoryFilters={true} />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white dark:bg-slate-900 p-4 border dark:border-slate-800 rounded-xl shadow-sm">
        <div>
          ...
          <p className="text-[11px] opacity-60">Filtered grid scope items: {currentFilteredView.length}</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className={`w-full max-w-lg p-6 rounded-2xl border shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200'}`}>
            ...
            <div className="space-y-4">
              <select value={meta.office} onChange={e=>setMeta({...meta, office:e.target.value, fd:'', section:''})} className={inp}>
                <option value="">Office</option>
      ...
      {editModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <form onSubmit={handleUpdateItem} className={`w-full max-w-md p-6 rounded-2xl border shadow-2xl space-y-4 ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200'}`}>
            ...
            <button type="submit" className="w-full py-2 bg-amber-500 hover:bg-amber-600 text-emerald-950 font-black text-xs rounded transition shadow-md">
...
function ContributionsWorkspace({ contributions, setContributions, userContext, darkMode }) {
  ...
  const [meta, setMeta] = useState({ partner: '', office: '', fd: '', section: '' });
  const [lines, setLines] = useState([]);
  const [workingItem, setWorkingItem] = useState({ specificItem: '', category: '', qty: '', uom: 'Piece (pc)', value: '' });
  ...
  const currentFilteredView = useMemo(() => {
    ...
    return scope.filter(c => {
      if (filters.level !== 'All' && ((filters.level === 'RO' && c.office !== 'Regional Office') || (filters.level === 'SDO' && c.office === 'Regional Office'))) return false;
  ...
  }, [contributions, filters, userContext]);
  ...
  const handleCommitBatch = (e) => {
    ...
    setLines([]);
    setMeta({ partner: '', office: '', fd: '', section: '' });
    setAddModal(false);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-white dark:bg-slate-900 p-4 border dark:border-slate-800 rounded-xl shadow-sm">
        <div>
          ...
          <p className="text-[11px] opacity-60">System indexed ledger records: {currentFilteredView.length}</p>
        </div>
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {!isReadOnly && (
          ...
          <tbody className="divide-y dark:divide-slate-800/60">
            {currentFilteredView.map(c => (
              <tr key={c.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                <td className="py-2.5 font-bold text-emerald-800 dark:text-amber-500">{c.partner}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <div className={`w-full max-w-lg p-6 rounded-2xl border shadow-2xl flex flex-col max-h-[90vh] overflow-y-auto ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200'}`}>
            ...
            <div className="space-y-4">
              ...
              <select value={meta.office} onChange={e=>setMeta({...meta, office:e.target.value, fd:'', section:''})} className={inp}><option value="">Office</option>{OFFICES.map(o=><option key={o} value={o}>{o}</option>)}</select>
              <div className="grid grid-cols-2 gap-2">
                <select value={meta.fd} onChange={e=>setMeta({...meta, fd:e.target.value, section:''})} className={inp} disabled={!meta.office}><option value="">Functional Division</option>{activeFds.map(f=><option key={f} value={f}>{f}</option>)}</select>
                <select value={meta.section} onChange={e=>setMeta({...meta, section:e.target.value})} className={inp} disabled={!meta.fd}><option value="">Section/Unit</option>{activeSections.map(s=><option key={s} value={s}>{s}</option>)}</select>
              </div>
      ...
      {isAddOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
          <form onSubmit={handleAddUser} className={`w-full max-w-md p-6 rounded-2xl border shadow-2xl space-y-3 ${darkMode ? 'bg-slate-900 border-slate-800 text-slate-100' : 'bg-white border-slate-200'}`}>
            ...
            <select value={form.office} onChange={e=>setForm({...form, office:e.target.value})} className={inp}><option value="All">Global Framework</option>{OFFICES.map(o=><option key={o} value={o}>{o}</option>)}</select>
